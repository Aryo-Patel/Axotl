...
router.post('/', async(req, res) => {
    ...
    try {
        ...
        if (attachment) { newPost.attachment = attachment }
        await newPost.save();
        res.json({ newPost });
    } catch (err) {
        console.error(err.message);
...
router.delete('/comment/:post_id/:comment_id', async(req, res) => {
    ...
    const { text } = req.body;
    try {
        await post.comments.filter(comment => comment._id.toString() != req.params.comment_id);
        res.json({ post });
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.put('/like/:post_id/', async(req, res) => {
    ...
    try {
        if (post.likes.filter(like => like._id.toString() == req.user._id.toString()).length != 0) {
...
router.put('/unlike/:post_id/', async(req, res) => {
    ...
    try {
        ...
        await post.likes.filter(like => like._id.toString() != req.user._id.toString())
        res.json({ post });
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
