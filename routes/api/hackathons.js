    ...
    try {
        ...
        if (donations) {
            ...
            donations.forEach((donation) => {
                ...
                const { type, quantity, description, received } = donation;
                ...
                if (received) {
                    ...
                    received.forEach((reception) => {
                        ...
                        const { sponsor, quantity, description } = reception;
        ...
        user.myHackathons.unshift(newHackathon._id);
        await user.save();
        res.json({ newHackathon, user });
    } catch (err) {
        console.error(err);
...
router.delete("/delete-hackathon/:hackathonId", async(req, res, next) => {
    ...
    user.myHackathons = await user.myHackathons.filter(
        (hackathon) => hackathon.toString() != hackathonId
    );
    await user.save();
...
router.get("/my-hackathons/:pageNumber", async(req, res, next) => {
    ...
    try {
        ...
        // console.log(req.user.myHackathons)
